...
interface KpiData {
  labelKey: 'kpiReceiving' | 'kpiShipping' | 'kpiAlert' | 'kpiPending'
  value: number
  unitKey: 'unitOrders' | 'unitItems'
  trend: string
  trendDir: 'up' | 'down' | 'flat'
  trendLabelKey: 'trendYesterday' | 'trendBelowMin' | 'trendActionNeeded'
...
const kpiStats: KpiData[] = [
  {
    labelKey: 'kpiReceiving', value: 12, unitKey: 'unitOrders',
    trend: '+3', trendDir: 'up', trendLabelKey: 'trendYesterday',
    icon: PackageCheck, accentColor: '#005B99', bgColor: '#E6F3F9', iconColor: '#005B99',
  },
  {
    labelKey: 'kpiShipping', value: 8, unitKey: 'unitOrders',
    trend: '-2', trendDir: 'down', trendLabelKey: 'trendYesterday',
    icon: PackageMinus, accentColor: '#00A0C8', bgColor: '#E6F8FB', iconColor: '#00A0C8',
  },
  {
    labelKey: 'kpiAlert', value: 3, unitKey: 'unitItems',
    trend: '±0', trendDir: 'flat', trendLabelKey: 'trendBelowMin',
    icon: AlertTriangle, accentColor: '#D97706', bgColor: '#FEF3C7', iconColor: '#D97706',
  },
  {
    labelKey: 'kpiPending', value: 5, unitKey: 'unitOrders',
    trend: '+2', trendDir: 'up', trendLabelKey: 'trendActionNeeded',
    icon: ClipboardList, accentColor: '#DC2626', bgColor: '#FEE2E2', iconColor: '#DC2626',
  },
]

function KpiCard({ stat }: { stat: KpiData }) {
      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-center gap-3 sm:block">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-slate-500 leading-snug mb-1 sm:mb-4 truncate">
              {t(stat.labelKey)}
            </p>
        ...
        <div className="flex items-center gap-1 mt-2 sm:mt-3">
          ...
          <span className="text-xs font-medium" style={{ color: trendColor }}>{stat.trend}</span>
          <span className="text-xs text-slate-400 truncate">{t(stat.trendLabelKey)}</span>
        </div>
      </div>
    </div>
  )
...
export default function DashboardPage() {
  ...
  const dateStr = today.toLocaleDateString(locale === 'en' ? 'en-US' : 'ja-JP', {
  ...
  return (
    <div className="space-y-6 max-w-screen-xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
        {kpiStats.map((stat) => (
          <KpiCard key={stat.labelKey} stat={stat} />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        {/* 最近の入庫 */}
        <TableCard title={t('recentReceiving')} href="/receiving" linkLabel={tc('viewAll')}>
          {/* モバイル：カード表示 */}
          ...
          <table className="w-full hidden sm:table">
            <thead>
              <tr style={{ borderBottom: '2px solid #E2E8F0', backgroundColor: '#F8FAFC' }}>
                <th className="px-5 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">{t('colReceivingNo')}</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide">{t('colSupplier')}</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">{t('colDate')}</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide">{t('colStatus')}</th>
              </tr>
            </thead>
            <tbody>
              {recentReceivings.map((r, i) => (
                <tr
                  ...
                  style={{ borderBottom: i < recentReceivings.length - 1 ? '1px solid #F1F5F9' : 'none' }}
                ...
                >
                  ...
                  <td className="px-4 py-3"><Badge label={r.status} styleMap={receivingStatusStyle} /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* 未処理の出庫指示 */}
        <TableCard title={t('pendingShipping')} href="/shipping" linkLabel={tc('viewAll')}>
          {/* モバイル：カード表示 */}
          <div className="sm:hidden divide-y divide-slate-100">
          ...
          <table className="w-full hidden sm:table">
            <thead>
              <tr style={{ borderBottom: '2px solid #E2E8F0', backgroundColor: '#F8FAFC' }}>
                <th className="px-5 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">{t('colShippingNo')}</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide">{t('colCustomer')}</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">{t('colShipDate')}</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide">{t('colStatus')}</th>
              </tr>
            </thead>
            <tbody>
              {pendingShippings.map((s, i) => (
                <tr
                  key={s.code}
                  className="transition-colors"
                  style={{ borderBottom: i < pendingShippings.length - 1 ? '1px solid #F1F5F9' : 'none' }}
                  onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = '#F0F7FB')}
                  onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = '')}
                >
                  <td className="px-5 py-3 whitespace-nowrap">
                  ...
                  <td className="px-4 py-3"><Badge label={s.status} styleMap={shippingStatusStyle} /></td>
                </tr>
              ))}
            </tbody>
          </table>
