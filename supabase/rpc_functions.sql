...
-- =============================================================================
-- 【ステータス制御】
--   pending のみ再引当可（必ずサーバー側でチェック）
--   picking / inspected / shipped / cancelled → 不可
--
-- 【パラメータ】
--   p_header_id    : shipping_headers.id（ステータス検証）
--   p_tenant_id    : tenants.id（スコープ検証）
--   p_warehouse_id : warehouses.id（スコープ検証）
...
-- =============================================================================
-- 6. rpc_cancel_shipping_order
--    出荷キャンセル：shipping_header を cancelled に更新し、
--    全 allocation を解除して inventory.allocated_qty を戻す。
--    単一トランザクションで全更新を実行する。
-- =============================================================================
-- 【ステータス制御】
--   pending / picking / inspected → キャンセル可（必ずサーバー側でチェック）
--   shipped                       → 不可（on_hand_qty 減算済み。別途入庫で対応）
--   cancelled                     → 冪等: 即 success を返す（二重キャンセル防止）
--
-- 【パラメータ】
--   p_header_id    : shipping_headers.id
--   p_tenant_id    : tenants.id（スコープ検証用）
--   p_warehouse_id : warehouses.id（スコープ検証用）
--   p_reason       : キャンセル理由（任意。inventory_transactions.note に記録）
--
-- 【更新対象】
--   shipping_allocations  : 全件 DELETE
--   inventory.allocated_qty : 各 allocation 分だけ減算（on_hand_qty は不変）
--   inventory_transactions  : type='deallocation', note='reason:cancel[:<p_reason>]'
--   shipping_lines          : status='cancelled', allocated_qty=0
--   shipping_headers        : status='cancelled'
--
-- 【ロック順序】
--   shipping_headers FOR UPDATE → shipping_lines FOR UPDATE (ORDER BY id ASC)
--   → shipping_allocations (ORDER BY id ASC) → inventory FOR UPDATE
--   既存 deallocate / reallocate RPC と同じロック戦略で deadlock を防止する。
